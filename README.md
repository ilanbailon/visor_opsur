...
Crea la tabla `marcaciones_adjuntos` con la siguiente estructura (ajusta el esquema si no usas `public`). Puedes ejecutar el
script `sql/marcaciones_adjuntos.sql` directamente desde la consola SQL de Supabase o cualquier cliente Postgres:
...
2. Cada archivo se sube a R2 y se registra en `marcaciones_adjuntos` con su tipo MIME original.
